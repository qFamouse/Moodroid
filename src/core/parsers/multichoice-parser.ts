...
export class MultichoiceParser implements IAnswerParser {
    parse(que: HTMLElement): MultichoiceAnswer {
        ...
        if (
            group == QuestionGroup.complete &&
            state == QuestionState.partiallycorrect
        ) {
            throw new Error(
                "Can't parse complete question in partiallycorrect state"
            )
        }

        return {
            correctAnswers: MultichoiceParser.parseAnswers(
                que,
                group,
                state,
                true
            ),
            incorrectAnswers: MultichoiceParser.parseAnswers(
                que,
                group,
                state,
                false
    ...
    private static parseAnswers(
        que: HTMLElement,
        group: QuestionGroup,
        state: QuestionState,
        parseCorrect: boolean
    ): string[] {
        ...
        switch (group) {
            case QuestionGroup.correctIncorrect:
                if (parseCorrect) {
                    answerLabels = que.querySelectorAll(
                        ".answer>[class^=r].correct>[data-region^=answer-label]>:last-child"
                    )
                } else {
                    answerLabels = que.querySelectorAll(
                        ".answer>[class^=r].incorrect>[data-region^=answer-label]>:last-child"
                    )
                }
                break
            ...
            case QuestionGroup.complete:
                ...
                if (
                    (parseCorrect && state == QuestionState.correct) ||
                    (!parseCorrect && state == QuestionState.incorrect)
                ) {
                    answerLabels = que.querySelectorAll(`
        ...
        return Array.from(answerLabels).map(
            (answerLabel) => answerLabel.textContent
        )
    }
}
