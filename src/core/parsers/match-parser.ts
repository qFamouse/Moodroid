...
export class MatchParser implements IAnswerParser {
    parse(que: HTMLElement): MatchAnswer {
        ...
        let rows: NodeListOf<HTMLElement> =
            que.querySelectorAll("tbody>[class^=r]")
            ...
            case QuestionGroup.correctIncorrect:
                rows.forEach((row) => {
                    ...
                    let correctAnswer: string =
                        row.querySelector(".correct>select>option:checked")
                            ?.textContent ?? undefined
                    let incorrectAnswer: string =
                        row.querySelector(".incorrect>select>option:checked")
                            ?.textContent ?? undefined

                    matchAnswer.answers[answerText] = {
                        correctAnswer: correctAnswer,
                        incorrectAnswers: incorrectAnswer
                            ? new Array(incorrectAnswer)
                            : []
                    }
                })
                return matchAnswer
                ...
                if (
                    ...
                    rows.forEach((row) => {
                        let answerText: string =
                            row.querySelector(".text").textContent
                        let answer: string =
                            row.querySelector("select>option:checked")
                                ?.textContent ?? undefined
                    ...
                    throw new Error(
                        "From this state of the question, it is not possible to find out the correct or incorrect answers"
                    )
                }
                return matchAnswer
